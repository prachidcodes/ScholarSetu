...
export const ApplicationsPage: React.FC = () => {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'Disbursed':
        return <Badge variant="success" dot>Disbursed</Badge>;
      case 'Sanctioned':
        return <Badge variant="purple" dot>Sanctioned</Badge>;
      case 'Under Manual Review':
        return <Badge variant="warning" dot>Under Manual Review</Badge>;
      case 'Under Verification':
        return <Badge variant="info" dot>Under Verification</Badge>;
      case 'Deficiency Found':
        return <Badge variant="error" dot>Deficiency Found</Badge>;
      default:
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 text-xs uppercase font-bold tracking-wider text-teal-800 mb-1">
            ...
            <span>Ministry Application Tracker</span>
          </div>
          <h1 className="font-display text-2xl sm:text-3xl font-black text-slate-900 tracking-tight">
            My Scholarship Applications
          </h1>
          <p className="text-xs sm:text-sm text-slate-600 mt-1 leading-relaxed">
            Live lifecycle tracking across automated verification, District Welfare Officer manual review, Sanction order, and DBT release.
          </p>
        </div>
      ...
      <div className="space-y-4">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                  ...
                  <div className="space-y-2 flex-1">
                    ...
                    <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-600">
                      <span><strong>Institution:</strong> {app.institutionName}</span>
                      <span><strong>Course:</strong> {app.course || app.courseName}</span>
                      <span><strong>Income:</strong> ₹{(app.annualIncome || app.submittedAnnualIncome || 0).toLocaleString('en-IN')}/yr</span>
                    </div>
                    ...
                    {hasMismatch && app.status !== 'Disbursed' && (
                      <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-amber-50 text-amber-900 text-xs font-semibold border border-amber-300">
                        ...
                        <span>Discrepancy: Income Verification Query (Readiness: {app.readinessScore}%)</span>
                      </div>
                    )}
                    ...
                    {app.status === 'Disbursed' && app.disbursementDetails && (
                      <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-lg bg-emerald-50 text-emerald-900 text-xs font-semibold border border-emerald-300">
                        ...
                        <span>DBT Credited: ₹{app.disbursementDetails.amount.toLocaleString('en-IN')} on {app.disbursementDetails.disbursedDate}</span>
                      </div>
                    )}
                  </div>
        ...
        {applications.length === 0 && (
          <div className="text-center py-12 bg-white rounded-xl border border-slate-200 p-8">
            ...
            <p className="text-slate-600 font-semibold text-sm">No applications submitted yet.</p>
            <Link to="/student/scholarships" className="mt-2 text-xs font-bold text-teal-800 underline block">
              Browse and apply for 5 Ministry Schemes →
            </Link>
          </div>
        )}
      </div>
